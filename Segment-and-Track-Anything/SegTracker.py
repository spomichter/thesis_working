...
class SegTracker():
    ...
    def seg_with_boxes(self,frame,boxes):
        '''
        Arguments:
            frame: numpy array (h,w,3)
            boxes: list of list [x0,y0,x1,y1]
        Return:
            origin_merged_mask: numpy array (h,w)
        '''
        frame = frame[:, :, ::-1]
        self.origin_merged_mask = np.zeros((frame.shape[0],frame.shape[1]),dtype=np.uint8)
        idx = 1
        for box in boxes:
            box = [int(i) for i in box]
            interactive_mask = self.sam.segment_with_raw_box(frame, box)[0]
            self.origin_merged_mask[interactive_mask==1] = idx
            idx += 1
        return self.origin_merged_mask
